...
export const metadata: Metadata = {
  ...
  description: "دليل طبي متكامل حول جراحات القولون والمستقيم، أنواع الاستئصال، فغر القولون، وفترات التعافي برعاية استشارية متخصصة.",
};
  ...
  return (
    <div className="space-y-12 md:space-y-20 pb-16">
      ...
      <section className="bg-navy-950/95 text-white py-14 md:py-20 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-right space-y-4">
          <span className="inline-block bg-accent-gold/20 text-accent-gold text-xs md:text-sm font-bold px-3.5 py-1.5 rounded-full border border-accent-gold/30">
            دليل طبي وجراحي متكامل
          </span>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white!">{colonSurgeryData.hero.title}</h1>
          <p className="text-base sm:text-lg text-slate-300 max-w-3xl leading-relaxed">
            {colonSurgeryData.hero.subtitle}
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 md:space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {colonSurgeryData.indicationsSection.items.map((item, idx) => (
              ...
              >
                ...
                <p className="text-xs text-slate-600 leading-relaxed pr-7">{item.text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-navy-950/95 text-white rounded-3xl p-6 md:p-10 shadow-xl space-y-6">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-bold text-white! border-r-4 border-accent-gold pr-3">
        ...
        <section className="bg-white rounded-3xl p-6 md:p-10 shadow-card border border-slate-100 space-y-6">
          <div className="space-y-2 border-r-4 border-brand pr-3">
            ...
            <p className="text-sm text-slate-600">{colonSurgeryData.colectomySection.intro}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {colonSurgeryData.colectomySection.subsections.map((sub, i) => (
              ...
              >
                <h3 className="font-bold text-brand-900 text-lg">{sub.title}</h3>
                <p className="text-xs text-slate-700 leading-relaxed">{sub.text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-white rounded-3xl p-6 md:p-10 shadow-card border border-slate-100 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {colonSurgeryData.colostomySection.types.map((colType, idx) => (
              ...
              >
                <h3 className="font-bold text-slate-900 text-base">{colType.title}</h3>
                <p className="text-xs text-slate-600 leading-relaxed">{colType.text}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-gradient-to-r from-amber-500/10 via-rose-50 to-amber-50 rounded-3xl p-6 md:p-10 border-2 border-amber-300 shadow-lg space-y-6">
          ...
          <div className="bg-white rounded-2xl p-5 border border-rose-200 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md">
            <div className="space-y-1 text-center sm:text-right">
              <span className="text-xs font-bold text-rose-600 uppercase">حالة عاجلة؟</span>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-7">
            <ContactForm
              ...
              subtitle="سنقوم بالتواصل معك وتقديم المشورة الطبية الإكلينيكية المناسبة."
            />
          </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-brand-900 text-white rounded-2xl p-6 shadow-card space-y-3">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                متاح بالعيادة أيام {doctorData.workingHours} بمجمع تداوي الجراحي الطبي بالطائف.
              </p>
              <Link
                href="/appointments"
